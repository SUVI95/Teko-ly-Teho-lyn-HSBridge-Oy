...
router.get('/status', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, employment_status, profession, biggest_challenge, ai_experience, ai_goals, ai_feeling, ai_summary, created_at
...
router.post('/complete', authenticateToken, async (req, res) => {
  try {
    ...
    if (!employment_status || !profession || !biggest_challenge || !ai_experience || goalsArr.length === 0 || !ai_feeling) {
    ...
    const result = await pool.query(
      `INSERT INTO user_onboarding (
        user_id, employment_status, profession, biggest_challenge, ai_experience, ai_goals, ai_feeling, ai_summary
