...
function isDeliverableEmail(addr) {
  const e = String(addr || '').trim();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)) return false;
  if (/@example\.(com|org|net|fi)$/i.test(e)) return false;
  return true;
}

function notifyEmailFor(row) {
  const account = String(row.account_email || '').trim();
  const pub = String(row.email_public || '').trim();
  if (isDeliverableEmail(account)) return account;
  if (isDeliverableEmail(pub)) return pub;
  return '';
}
...
async function notifyVisit(slug) {
  ...
  if (!sent.ok || sent.stub) {
    console.error('Visit notify email failed:', sent.error || (sent.stub ? 'RESEND_API_KEY missing' : 'unknown'));
    return;
  }
...
async function notifyContact(slug, { name, email, message }) {
  ...
  if (!sent.ok) {
    console.error('Contact notify email failed:', sent.error || 'unknown');
    return { ok: false, error: 'Sähköpostin lähetys epäonnistui. Yritä myöhemmin uudelleen.' };
  }
  if (sent.stub) {
    console.error('Contact notify email stubbed — RESEND_API_KEY not configured');
    return { ok: false, error: 'Sähköpostipalvelu ei ole käytössä palvelimella.' };
  }
